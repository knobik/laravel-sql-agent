...
it('can toggle sql panel', function () {
    $user = Helpers::createTestUser();
    Helpers::actingAs($user);

    \Livewire\Livewire::test(ChatComponent::class)
        ->set('currentSql', 'SELECT * FROM users')
        ->call('toggleSqlPanel')
        ->assertSet('showSqlPanel', true)
        ->call('toggleSqlPanel')
        ->assertSet('showSqlPanel', false);
});

it('can close sql panel', function () {
    $user = Helpers::createTestUser();
    Helpers::actingAs($user);

    \Livewire\Livewire::test(ChatComponent::class)
        ->set('showSqlPanel', true)
        ->call('closeSqlPanel')
        ->assertSet('showSqlPanel', false);
});
